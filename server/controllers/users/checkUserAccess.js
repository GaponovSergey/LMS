...
export default async function checkUserAccess(req, res, next) {
        try {
            ...
            if (req.session.user.account.access === 2 && req.session.user.account.id !== req.body.userId) {
            ...
            const user = await User.findByPk(req.session.user.account.id);

            if (!user) {
                throw new ValidationError("Нет данных пользователя");
            }

            const password = (req.body.password) ? argon2.hash(req.body.password) : req.session.user.account.password || null;
